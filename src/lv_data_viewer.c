...
static void graph_tab_create(lv_obj_t* parent);
...
static void slider_color_event_cb(lv_event_t* e);
static lv_obj_t* slider_color_label;
static void event_handler_red(lv_event_t* e);
static void event_handler_green(lv_event_t* e);
static void event_handler_blue(lv_event_t* e);
...
void lv_data_viewer(void)
{
    ...
    lv_obj_t* t1 = lv_tabview_add_tab(tv, "Logs");
    lv_obj_t* t2 = lv_tabview_add_tab(tv, "Graph");
...
static void event_button_domain_cb(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        {
            lv_obj_add_flag(chart_freq, LV_OBJ_FLAG_HIDDEN);
            lv_obj_remove_flag(chart_time, LV_OBJ_FLAG_HIDDEN);
        ...
        else
        {
            ...
            lv_obj_remove_flag(chart_freq, LV_OBJ_FLAG_HIDDEN);
...
static void chart_tab_create(lv_obj_t* parent)
{
    ...
    //lv_obj_set_flex_grow(button_domain, 1);
    ...
    lv_chart_set_point_count(chart_freq, MY_DATA_SIZE);
    ...
    //lv_obj_set_style_pad_hor(scale_bottom, lv_chart_get_first_point_center_offset(chart_freq), 0);

    volatile uint32_t chart_width = lv_chart_get_point_count(chart_freq);
    /*volatile uint32_t chart_height = lv_chart_set_range(;*/

    /*Add two data series*/
    lv_chart_series_t* my_ser1 = lv_chart_add_series(chart_freq, lv_palette_main(LV_PALETTE_GREEN), LV_CHART_AXIS_PRIMARY_Y);
    lv_chart_series_t* my_ser2 = lv_chart_add_series(chart_freq, lv_palette_main(LV_PALETTE_RED), LV_CHART_AXIS_SECONDARY_Y);

    ///*Create a transparent wrapper for the chart and the scale.
    //*Set a large width, to make it scrollable on the main container*/
    //lv_obj_t* wrapper2 = lv_obj_create(main_cont);
    //lv_obj_remove_style_all(wrapper2);
    //lv_obj_set_size(wrapper2, lv_pct(430), lv_pct(375));
    ...
    //lv_obj_add_flag(chart_time, LV_OBJ_FLAG_FLEX_IN_NEW_TRACK);

    /////*Create a scale also with 100% width*/
    ////lv_obj_t* scale_bottom2 = lv_scale_create(wrapper2);
    ////lv_scale_set_mode(scale_bottom2, LV_SCALE_MODE_HORIZONTAL_BOTTOM);
    ////lv_obj_set_size(scale_bottom2, lv_pct(100), 25);
    ////lv_scale_set_total_tick_count(scale_bottom2, 12);
    ////lv_scale_set_major_tick_every(scale_bottom2, 1);
    ////lv_obj_set_style_pad_hor(scale_bottom2, lv_chart_get_first_point_center_offset(chart_time), 0);

    lv_chart_series_t* my_ser3 = lv_chart_add_series(chart_time, lv_palette_main(LV_PALETTE_BLUE), LV_CHART_AXIS_SECONDARY_Y);
    lv_chart_series_t* my_ser4 = lv_chart_add_series(chart_time, lv_palette_main(LV_PALETTE_DEEP_ORANGE), LV_CHART_AXIS_SECONDARY_Y);



    //static uint32_t * data_buffer1[MY_DATA_SIZE];
    //static int32_t * data_buffer2[MY_DATA_SIZE];

    //display sine /cosine or 2*sine
    int32_t i;
    for (i = 0; i < MY_DATA_SIZE; i++) {
#if (CHART_TYPE == LV_CHART_TYPE_LINE)
        //my_ser1->y_points[i] = lv_rand(CHART1_MIN, CHART1_MAX);
        my_ser1->y_points[i] = lv_trigo_sin(i) > 0 ? lv_trigo_sin(i) / 327 : -(lv_trigo_sin(i) / 327);
        //my_ser2->y_points[i] = lv_rand(CHART2_MIN, CHART2_MAX);
        my_ser2->y_points[i] = lv_trigo_sin(i-90) > 0 ? lv_trigo_sin(i-90) / 327 : -(lv_trigo_sin(i-90) / 327);
        //my_ser3->y_points[i] = lv_rand(CHART3_MIN, CHART3_MAX);
        my_ser3->y_points[i] = lv_trigo_sin(i - 225) > 0 ? lv_trigo_sin(i - 225) / 327 : -(lv_trigo_sin(i - 225) / 327);
        //my_ser4->y_points[i] = lv_rand(CHART4_MIN, CHART4_MAX);
        my_ser4->y_points[i] = lv_trigo_sin(i - 135) > 0 ? lv_trigo_sin(i - 135) / 327 : -(lv_trigo_sin(i - 135) / 327);

#elif (CHART_TYPE == LV_CHART_TYPE_SCATTER)
        lv_chart_set_next_value2(chart, my_ser1, i, lv_rand(0, 25));
        lv_chart_set_next_value2(chart, my_ser2, i, lv_rand(25, 50));
        lv_chart_set_next_value2(chart, my_ser3, i, lv_rand(50, 75));
        lv_chart_set_next_value2(chart, my_ser4, i, lv_rand(75, 100));
#endif

    }

    //lv_chart_set_ext_y_array(chart, ser1, &data_buffer1);
    //lv_chart_set_ext_y_array(chart, ser2, &data_buffer2);
...
typedef struct {
    uint8_t r;       // an integer between 0 and 255
    uint8_t g;       // an integer between 0 and 255
    uint8_t b;       // an integer between 0 and 255
} rgb;

typedef struct {
    uint16_t h;       // angle in degrees, 
    uint8_t s;       // an integer between 0 and 255
    uint8_t l;       // an integer between 0 and 255
} hsl;

static hsl   rgb2hsl(rgb in);
static rgb   hsl2rgb(hsl in);

hsl rgb2hsl(rgb in)
{
    hsl         out;
    uint16_t      min, max, rangec, sumc;
    uint16_t new_r = (uint16_t)in.r;
    uint16_t new_g = (uint16_t)in.g;
    uint16_t new_b = (uint16_t)in.b;

    //find minimum by comparing R to G, then whichever was less to B
    min = new_r < new_g ? new_r : new_g;
    min = min < new_b ? min : new_b;

    //find maximum by comparing R to G, then whichever was more to B
    max = new_r > new_g ? new_r : new_g;
    max = max > new_b ? max : new_b;

    //calculate the range and sum between minimum and maximum
    rangec = max - min;
    sumc = max + min;

    //calculate luminance, l 
    out.l = (uint8_t)(sumc / 2);

    //if the color is gray (all RGB are equal) then no saturation or hue
    if (min == max)
    {
        out.s = (uint8_t)0;
    ...
    else
    {
        ...
        if (out.l <= 128)
        {
            out.s = (uint8_t)((255 * rangec) / sumc);
        }
        else
        {
            out.s = (uint8_t)(255 * rangec / (512 - sumc));
        }



        int16_t g_b = new_g - new_b;
        int16_t b_r = new_b - new_r;
        int16_t r_g = new_r - new_g;
        if (rangec == 0u)
        {
            out.h = 0u;
        }
        ...
        {
            out.h = (uint16_t)((0 + ((60 * (g_b)) / rangec)) % 360);
        }
        else if (max == new_g)
        {
            out.h = (uint16_t)((120 + ((60 * (b_r)) / rangec)) % 360);
        }
        else if (max == new_b)
        {
            out.h = (uint16_t)((240 + ((60 * (r_g)) / rangec)) % 360);
        }
...
rgb hsl2rgb(hsl in)
{
    ...
    int16_t temp1 = 2 * in.l - 255;
    if (temp1 < 0)
    {
        temp1 = temp1 * -1;
    }
    int16_t C_val = (temp1 - 255) * in.s / 255;
    if (C_val < 0)
    {
        C_val = C_val * -1;
    }

    int16_t temp2 = (in.h) % 120 - 60;
    if (temp2 < 0)
    {
        temp2 = temp2 * -1;
    }

    int16_t X_val = (C_val * 60 * (60 - temp2)) / 3600;

    int16_t m_val = in.l - C_val / 2;
...
static lv_obj_t* create_chart_with_scales(lv_obj_t* parent, const char* title, const char* hor_text[])
{
    ...
    lv_scale_set_total_tick_count(scale_ver, MY_DATA_SIZE);
    lv_scale_set_major_tick_every(scale_ver, MY_DATA_SIZE/10);
...
static void graph_tab_create(lv_obj_t* parent)
{
    ...
    static const char* chart1_texts[] =
    { "Jan", "Feb", "March", "April", "May", "Jun", "July", "Aug", "Sept", "Oct", "Nov", "Dec", NULL };
...
void fade_create(lv_obj_t* parent)
{

    lv_obj_t* label;

    static int32_t col_dsc[] = { 75, 75, 75, 75, LV_GRID_TEMPLATE_LAST };
    static int32_t row_dsc[] = { 125, 100, 100, 100, LV_GRID_TEMPLATE_LAST };

    /*Create a container with grid*/
    lv_obj_t* cont = lv_obj_create(parent);
    lv_obj_set_grid_dsc_array(cont, col_dsc, row_dsc);
    lv_obj_set_size(cont, 350, 450);
    lv_obj_center(cont);

    lv_obj_t* obj;

    /*Cell to r0, c0-4 .Set width and height to stretched.*/
    ...
    lv_obj_t* slider_color = lv_slider_create(obj);
    lv_obj_center(slider_color);
    lv_obj_add_event_cb(slider_color, slider_color_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_set_size(slider_color, 250, 60);

    lv_obj_set_style_anim_duration(slider_color, 2000, 0);
    /*Create a label below the slider*/
    slider_color_label = lv_label_create(obj);
    lv_label_set_text(slider_color_label, "0%");
    //lv_obj_align_to(slider_color_label, slider_color, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

    /*Cell to r1-4, c1-4 .Set width and height to stretched.*/
    obj = lv_obj_create(cont);
    lv_obj_set_size(obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    lv_obj_set_grid_cell(obj, LV_GRID_ALIGN_STRETCH, 0, 4,
        LV_GRID_ALIGN_STRETCH, 1, 3);
    //lv_obj_t* bar1 = lv_bar_create(parent);
    bar1 = lv_bar_create(obj);
    lv_obj_set_size(bar1, 250, 250);
    lv_obj_center(bar1);
    lv_bar_set_value(bar1, 100, LV_ANIM_OFF);
    lv_style_set_radius(&fade_bar_style, 0);
    lv_style_init(&fade_bar_style);

    ///*Cell to r1, c0 .Set width and height to stretched.*/
    //obj = lv_obj_create(cont);
    //lv_obj_set_size(obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    //lv_obj_set_grid_cell(obj, LV_GRID_ALIGN_STRETCH, 0, 1,
    //    LV_GRID_ALIGN_STRETCH, 1, 1);

    //lv_obj_t* btn_red = lv_button_create(obj);
    //lv_obj_add_event_cb(btn_red, event_handler_red, LV_EVENT_ALL, NULL);
    //lv_obj_align(btn_red, LV_ALIGN_CENTER, 0, 0);
    //lv_obj_add_flag(btn_red, LV_OBJ_FLAG_CHECKABLE);
    //lv_obj_set_height(btn_red, LV_SIZE_CONTENT);
    //label = lv_label_create(btn_red);
    //lv_label_set_text(label, "Red");
    //lv_obj_center(label);


    ///*Cell to r2, c0 .Set width and height to stretched.*/
    //obj = lv_obj_create(cont);
    //lv_obj_set_size(obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    //lv_obj_set_grid_cell(obj, LV_GRID_ALIGN_STRETCH, 0, 1,
    //    LV_GRID_ALIGN_STRETCH, 2, 1);
    //lv_obj_t* btn_green = lv_button_create(obj);
    //lv_obj_add_event_cb(btn_green, event_handler_green, LV_EVENT_ALL, NULL);
    //lv_obj_align(btn_green, LV_ALIGN_CENTER, 0, 0);
    //lv_obj_add_flag(btn_green, LV_OBJ_FLAG_CHECKABLE);
    //lv_obj_set_height(btn_green, LV_SIZE_CONTENT);
    //label = lv_label_create(btn_green);
    //lv_label_set_text(label, "green");
    //lv_obj_center(label);


    ///*Cell to r3, c0 .Set width and height to stretched.*/
    //obj = lv_obj_create(cont);
    //lv_obj_set_size(obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    //lv_obj_set_grid_cell(obj, LV_GRID_ALIGN_STRETCH, 0, 1,
    //    LV_GRID_ALIGN_STRETCH, 3, 1);
    //lv_obj_t* btn_blue = lv_button_create(obj);
    //lv_obj_add_event_cb(btn_blue, event_handler_blue, LV_EVENT_ALL, NULL);
    //lv_obj_align(btn_blue, LV_ALIGN_CENTER, 0, 0);
    //lv_obj_add_flag(btn_blue, LV_OBJ_FLAG_CHECKABLE);
    //lv_obj_set_height(btn_blue, LV_SIZE_CONTENT);
    //label = lv_label_create(btn_blue);
...
static void slider_color_event_cb(lv_event_t* e)
{
    lv_obj_t* slider_color = lv_event_get_target(e);
    char buf[8];
    lv_snprintf(buf, sizeof(buf), "%d%%", (int)lv_slider_get_value(slider_color));
    lv_label_set_text(slider_color_label, buf);
    lv_obj_align_to(slider_color_label, slider_color, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);

    //https://stackoverflow.com/questions/3018313/algorithm-to-convert-rgb-to-hsv-and-hsv-to-rgb-in-range-0-255-for-both
    //change color with slider.
    //  1. Read RGB from input
    //  2. Convert RGB to HSL (Hue, Saturation, Luminance)
    //  3. Read Luminance percent from slider
    //  4. Adjust luminance value via the slider
    //rgb my_rgb;
    //my_rgb.r = 0xFF*toggle_red;
    //my_rgb.g = 0xFF*toggle_green;
    //my_rgb.b = 0xFF*toggle_blue;

    hsl my_hsl;  //rgb2hsl(my_rgb);
    my_hsl.h = 0;
    my_hsl.s = 240u;
    my_hsl.l = 128;
    //my_hsl.l = (lv_slider_get_value(slider_color)*255) / 100;
    my_hsl.h = (lv_slider_get_value(slider_color) * 360) / 100;
    rgb new_rgb = hsl2rgb(my_hsl);

    uint32_t rgb_integer = (new_rgb.r) << 16 | (new_rgb.g) << 8 | (new_rgb.b);

    lv_style_set_bg_color(&fade_bar_style, lv_color_hex(rgb_integer));
    lv_style_set_border_width(&fade_bar_style, 2);

    lv_obj_add_style(bar1, &fade_bar_style, LV_PART_INDICATOR); /*Equal to LV_PART_MAIN | LV_STATE_DEFAULT*/

    /*Change the active screen's background color*/
    //lv_obj_set_style_bg_color(lv_screen_active(), lv_color_hex(rgb_integer), LV_PART_MAIN);
}
